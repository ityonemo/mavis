defmodule TypeTest.TypeBitString.UsableAsTest do
  ...
  describe "the empty bitstring type" do
    ...
    test "is not usable as a nonzero size bitstring" do
      ...
      assert {:error, %Message{challenge: @empty_bitstring, target: ^nonzero_bitstring}} =
  ...
  describe "fixed length bitstrings" do
    ...
    test "are not usable as other fixed-length bitstrings" do
      ...
      assert {:error, %Message{challenge: ^char2, target: ^char1}} = char2 ~> char1
    end
  end

  describe "the basic bitstring type" do
    test "is maybe usable as all other bitstrings" do
      assert {:maybe, [%Message{challenge: @basic_bitstring, target: @empty_bitstring}]} =
        @basic_bitstring ~> @empty_bitstring

      assert {:maybe, [%Message{challenge: @basic_bitstring, target: @basic_binary}]} =
        @basic_bitstring ~> @basic_binary

      assert {:maybe, [%Message{challenge: @basic_bitstring, target: %Bitstring{size: 7, unit: 2}}]} =
        @basic_bitstring ~> %Bitstring{size: 7, unit: 2}

      assert {:maybe, [%Message{challenge: @basic_bitstring, target: %Bitstring{size: 3, unit: 1}}]} =
  ...
  describe "for the basic binary type" do
    ...
    test "is maybe usable as some things" do
      ...
      assert {:maybe, [%Message{challenge: @basic_binary, target: %Bitstring{size: 8, unit: 0}}]} =
        @basic_binary ~> %Bitstring{size: 8, unit: 0}
      # competely strange factors that sometimes multiple to 8
      assert {:maybe, [%Message{challenge: @basic_binary, target: %Bitstring{size: 3, unit: 5}}]} =
        @basic_binary ~> %Bitstring{size: 3, unit: 5}
    end

    test "is never usable as something which can never be a non-multiple-of-8" do
      assert {:error, %Message{challenge: @basic_binary, target: %Bitstring{size: 3, unit: 4}}} =
  ...
  describe "corner cases:" do
    test "non-power-of-two" do
      ...
      assert {:maybe, [%Message{challenge: ^bs_1, target: ^bs_2}]} = bs_1 ~> bs_2
    end
  ...
  test "bitstrings generally are not usable as other types" do
    ...
    Enum.each(targets, fn target ->
      assert {:error, %Message{challenge: @empty_bitstring, target: ^target}} =
        (@empty_bitstring ~> target)
    end)

    Enum.each(targets, fn target ->
      assert {:error, %Message{challenge: @basic_binary, target: ^target}} =
        (@basic_binary ~> target)
    end)

    Enum.each(targets, fn target ->
      assert {:error, %Message{challenge: @basic_bitstring, target: ^target}} =
